...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-07-2020 a las 02:29:56
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_GAME` (IN `p_USERNAME` VARCHAR(25), IN `p_TITLE` VARCHAR(50), IN `p_GENRE` VARCHAR(50), IN `p_PUBLISHER` VARCHAR(50), IN `p_DEVELOPER` VARCHAR(50), IN `p_PLATFORM` VARCHAR(50), IN `p_RELEASED_ON` INT(4), IN `p_MULTIPLAYER` VARCHAR(25), IN `p_DESCRIPTION` VARCHAR(1000), IN `p_IMAGE` VARCHAR(100), IN `p_VIDEO_LINK` VARCHAR(50))  INSERT INTO GAME (ID_GAME, USERNAME, TITLE, GENRE, PUBLISHER, DEVELOPER, PLATFORM, RELEASED_ON, MULTIPLAYER, DESCRIPTION, IMAGE, VIDEO_LINK)
VALUES (DEFAULT, p_USERNAME, p_TITLE, p_GENRE, p_PUBLISHER, p_DEVELOPER, p_PLATFORM, p_RELEASED_ON, p_MULTIPLAYER, p_DESCRIPTION, p_IMAGE, p_VIDEO_LINK)$$
...
CREATE TABLE `game` (
  ...
  `GENRE` varchar(25) COLLATE utf8_unicode_ci NOT NULL DEFAULT 'NONE',
...
(4, 'admin', 'Horizon Zero Dawn', 'Action role-playing', 'Sony Interactive Entertai', 'Guerrilla Games', 'PlayStation 4', 2017, 'NO', 'The story is set in the 31st century, in a world where humans live in scattered tribal-like populations with limited access to technology. Their technologically advanced predecessors are remembered as the \"Old Ones\". Large robotic creatures, known as \"machines\", dominate the Earth. For the most part, they peacefully coexist with humans, who occasionally hunt them for parts. ', '../uploads/Horizon_ps4.jpg', 'https://www.youtube.com/embed/wzx96gYA8ek?rel=0'),
...
(8, 'admin', 'Devil May Cry 5', 'Hack and slash, Third-per', 'Capcom', 'Capcom', 'Microsoft Windows, PlayStation 4, Xbox One', 2019, 'YES', 'The gameplay features the return of Dante and Nero as playable characters, along with a new character, named \\\"V\\\". The gameplay is similar to that of the other titles in the Devil May Cry series, focusing on fast-paced \\\"stylish action\\\"; The player fights off hordes of demons with a variety of attacks and weapons. They receive a style-rating for combat based on a number of factors, such as move variety, the length of a combo and dodging attacks.', '../uploads/DMC5_20200707020817.jpg', 'https://www.youtube.com/embed/p2lWUQHUapM?rel=0'),
(9, 'admin', 'The Legend of Zelda: Breath of the Wild', 'Action-adventure', 'Nintendo', 'Nintendo EPD', 'Nintendo Switch, Wii U', 2017, 'NO', 'Breath of the Wild is an action-adventure game set in an open world where players are tasked with exploring the kingdom of Hyrule while controlling Link. In terms of structure, Breath of the Wild encourages nonlinear gameplay, which is illustrated by the game\\\'s lack of defined entrances or exits to areas, scant instruction given to the player, and encouragement to explore freely.', '../uploads/BOTW-thumb_20200707021425.jpg', 'https://www.youtube.com/embed/ofH5ptn5w-A?rel=0');
...
--
ALTER TABLE `game`
  MODIFY `ID_GAME` int(5) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
COMMIT;
